...
TEST_F(HashTableTest, updateEntryPayload) {
    for (int i = 0; i < this->numOfEntries; ++i) {
        ...
        int n1 = 57;
        p.setPayloadEntry(n1,NULL);

        errorVal = this->hTable->updateEntryPayload(&entryPtrs[i]->getWord(), p, MT_EXACT_MATCH, 0);
        ASSERT_TRUE(errorVal == H_T_SUCCESS);//succesful update

        Entry * ePtrHTable = this->hTable->getEntry(&this->entryPtrs[i]->getWord());
        List <PayloadEntry> payload2(ePtrHTable->getPayload(MT_EXACT_MATCH, 0));

        List <PayloadEntry> payload1;
        p.setPayloadEntry(i,NULL);
        payload1.insert(p);
        int n2 = i+1;
        p.setPayloadEntry(n2,NULL);
        payload1.insert(p);
        p.setPayloadEntry(n1,NULL);
        payload1.insert(p);
...
TEST(BucketTest, getEntry) {
    ...
    for (int i = 0; i < 16; ++i) {
        PayloadEntry p(i,NULL);
        Entry e(strArr[i], p, MT_EXACT_MATCH, 0);
